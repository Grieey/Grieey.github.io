...
第一步完成后，查看`branch1`上的提交，可以看到，最新的`commit`后面是`master`的`commit`，所以这里的实际情况和上面的动图有一些不一样在于，这一步之后，`branch1`上的`commits`就已经变成了整合了 `master`上的`commit`的一个分支，现在的`branch1`已经是拥有了两个分支完整的`commit`了。但是，我们需要的是`master`分支更完整。现在回到`master`分支，再执行`merge`命令就可以理解为什么是一次`fast-forward`了。最后的结果如下:
