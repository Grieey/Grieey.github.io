...
这里有一篇文章有基本的教程[传送门](https://www.itrhx.com/2019/08/01/A27-image-hosting/)，但是里面有一点细节没有说，那就是如果不需要发布**release版本来控制图片资源**，在**PicGo**中设置自定义Url的时候需要在仓库名后加上分支的名字，如`xxxxx/Github用户名/图床仓库名@仓库分支/路径`，而且仓库需要是`Public`状态，这样才能访问到我们放在仓库中的资源，否则会一直报错`failed to fetch xxxx`，。
...
在**merge**或者**rebase**中，出现该错误是因为两个分支没有取得联系，解决方案是在命令后面增加`--allow-unrelated-histories`,如
